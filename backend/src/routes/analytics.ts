...
router.get("/overview", async (c) => {
  ...
  const [totalShiftsThisWeek, checkins, activeEmployeesCount, anomalyCount] = await Promise.all([
    prisma.shift.count({
      where: { restaurantId: user.restaurantId, startTime: { gte: startOfWeek } },
    }),
    prisma.checkin.findMany({
      where: { restaurantId: user.restaurantId, checkinTime: { gte: startOfWeek } },
    }),
    prisma.employee.count({
      where: { restaurantId: user.restaurantId, isActive: true },
    }),
    prisma.anomaly.count({
      where: { restaurantId: user.restaurantId, isResolved: false },
    }),
  ]);
  ...
  const totalAssignments = await prisma.shiftAssignment.count({
    where: {
      shift: { restaurantId: user.restaurantId, startTime: { gte: startOfWeek } },
    },
  });
...
router.get("/workload-forecast", async (c) => {
  ...
  return c.json({ data: { slots: slots.filter((s) => s.shortage > 0 || s.assigned > 0) } });
});
...
router.get("/insights", async (c) => {
  ...
  const [employees, shifts, checkins, anomalies] = await Promise.all([
    ...
    prisma.shift.findMany({
      where: { restaurantId: user.restaurantId, startTime: { gte: lastMonth } },
    }),
    prisma.checkin.findMany({
      where: { restaurantId: user.restaurantId, checkinTime: { gte: lastMonth } },
    }),
    prisma.anomaly.findMany({
      where: { restaurantId: user.restaurantId, detectedAt: { gte: lastMonth } },
    }),
  ]);
